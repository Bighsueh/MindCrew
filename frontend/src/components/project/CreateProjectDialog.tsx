...
// specs/16-timer-system.md：建立專案時 timer preset 選項。
// 對齊後端 backend/app/timer/calculator.py 的 DEFAULT_2HR_PRESET / PRESET_4HR。
const TIMER_PRESETS: Record<string, TimerConfigInput> = {
  timer_preset_2hr: {
...
type TimerMode = 'preset_2hr' | 'preset_4hr' | 'custom'
...
export function CreateProjectDialog({
  ...
  const reset = () => {
    ...
    setCustomMacroBudgets({ discover: 45, define: 30, develop: 25, deliver: 20 })
    setError('')
    setIsCreating(false)
  ...
  // specs/16-timer-system.md：根據 timerMode 組出送往後端的 timer_config payload
  // （或 undefined 表示走後端預設）。
  const buildTimerConfig = (): TimerConfigInput | undefined => {
    if (timerMode === 'preset_2hr') return TIMER_PRESETS.timer_preset_2hr
    if (timerMode === 'preset_4hr') return TIMER_PRESETS.timer_preset_4hr
    const total =
        ...
        {step === 'basics' && (
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              {timerMode !== 'custom' && (
                <div className="rounded-md bg-bg-warm/60 px-3 py-2 text-xs text-text-muted">
                  Discover {TIMER_PRESETS[`timer_${timerMode}` as keyof typeof TIMER_PRESETS].macro_budgets.discover} 分 ·
                  Define {TIMER_PRESETS[`timer_${timerMode}` as keyof typeof TIMER_PRESETS].macro_budgets.define} 分 ·
                  Develop {TIMER_PRESETS[`timer_${timerMode}` as keyof typeof TIMER_PRESETS].macro_budgets.develop} 分 ·
                  Deliver {TIMER_PRESETS[`timer_${timerMode}` as keyof typeof TIMER_PRESETS].macro_budgets.deliver} 分
                </div>
              )}
            </div>

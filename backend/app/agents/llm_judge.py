...
_RULE_PROMPTS: dict[str, str] = {
    "no_interpretation": """你正在審查一張寫在 1-5「原始觀察 Raw Wall」區的便條紙。
本階段禁止對使用者行為做歸因 / 解讀 / 推論，只能保留原話 + 情緒 + 來源。

請判斷以下文字是否違規：
- pass：純粹引用使用者原話、描述觀察到的行為、引述他人話、Meta 討論（討論規則本身）
- violate：明顯把使用者的需求 / 想法 / 動機解讀出來（例：「使用者真正想要的是 X」「他其實是想 Y」「我覺得他的意思是 Z」）
- borderline：難以判斷時""",

    "empathy_says_no_inference": """你正在審查一張寫在 1-6 Empathy Map「Says」象限的便條紙。
Says 必須是使用者實際說過的話（引用），不能是推論。

請判斷以下文字是否違規：
- pass：明確的引用（含引號 / 「」/ quote 標記）、引述使用者說過的具體話語
- violate：寫使用者「覺得 / 認為 / 應該 / 可能 / 想要」這類推論（這些屬於 Thinks 象限）
- borderline：難以判斷時""",
...
- borderline：難以判斷時""",
    ...
    "no_feasibility_talk": """你正在審查 Develop（Phase 3）階段的便條紙或聊天訊息。
Phase 3 是純發散，禁止可行性討論（技術 / 成本 / 時間 / 實際性）。
...
DT 規則：禁止批評他人的想法（含委婉批評），有疑慮應留待收斂階段以投票表達。
